...
export default function Employees() {
  ...
  const getRoleColor = (r: string) => {
    if (r === 'admin') return isDarkMode ? 'bg-purple-500/20 text-purple-400' : 'bg-purple-100 text-purple-700';
    if (r === 'manager') return isDarkMode ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-700';
    if (r === 'seller') return isDarkMode ? 'bg-green-500/20 text-green-400' : 'bg-green-100 text-green-700';
    ...
    <Layout>
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className={'text-2xl font-bold ' + (isDarkMode ? 'text-white' : 'text-gray-900')}>Funcionarios</h1>
            <p className={'text-sm ' + (isDarkMode ? 'text-gray-400' : 'text-gray-600')}>Gerencie os funcionarios da sua empresa</p>
          </div>
          <div className="flex gap-3">
            ...
            <button onClick={() => { if (!canAdd()) { showMsg('error', 'Limite atingido'); return; } setEditingEmployee(null); resetForm(); setShowModal(true); }} className={'flex items-center gap-2 px-4 py-2 rounded-xl ' + (canAdd() ? 'bg-gradient-to-r from-cyan-500 to-purple-500 text-white hover:opacity-90' : 'bg-gray-500/20 text-gray-500 cursor-not-allowed')}>
              <PlusIcon size={18} /> Novo Funcionario
            </button>
          </div>
        </div>
        ...
        {userInfo && (
          <div className={'mb-6 p-4 rounded-xl flex items-center justify-between ' + (isDarkMode ? 'bg-white/5 border border-white/10' : 'bg-gray-50 border border-gray-200')}>
            <div className="flex items-center gap-3">
              <div className={'p-2 rounded-lg ' + (isDarkMode ? 'bg-purple-500/20' : 'bg-purple-100')}><UsersIcon size={20} className={isDarkMode ? 'text-purple-400' : 'text-purple-600'} /></div>
              <div><p className={'text-sm font-medium ' + (isDarkMode ? 'text-white' : 'text-gray-900')}>Funcionarios cadastrados</p><p className={'text-xs ' + (isDarkMode ? 'text-gray-400' : 'text-gray-500')}>Limite do seu plano</p></div>
            </div>
        ...
        {message && <div className={'mb-4 p-4 rounded-xl flex items-center gap-3 ' + (message.type === 'success' ? (isDarkMode ? 'bg-green-500/20 text-green-400' : 'bg-green-100 text-green-700') : (isDarkMode ? 'bg-red-500/20 text-red-400' : 'bg-red-100 text-red-700'))}><AlertIcon size={20} /> {message.text}</div>}

        {loading ? <div className={'text-center py-8 ' + (isDarkMode ? 'text-gray-400' : 'text-gray-600')}><RefreshIcon size={24} className="animate-spin mx-auto mb-2" /> Carregando...</div>
        : employees.length === 0 ? (
          <div className={'text-center py-16 rounded-2xl border ' + (isDarkMode ? 'bg-white/5 border-white/10 text-gray-400' : 'bg-white border-gray-200 text-gray-500')}>
            <UsersIcon size={48} className="mx-auto mb-4 opacity-50" /><p className="mb-4">Nenhum funcionario cadastrado</p>
            <button onClick={() => { setEditingEmployee(null); resetForm(); setShowModal(true); }} className="text-cyan-500 hover:text-cyan-400 font-medium">Cadastrar primeiro funcionario</button>
          </div>
        ) : (
          <div className={'rounded-2xl border overflow-hidden ' + (isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white border-gray-200 shadow-sm')}>
            <table className="min-w-full">
              <thead className={isDarkMode ? 'bg-white/5' : 'bg-gray-50'}>
                <tr>
                  <th className={'px-6 py-3 text-xs font-medium uppercase text-left ' + (isDarkMode ? 'text-gray-400' : 'text-gray-500')}>Usuario</th>
                  ...
                  <th className={'px-6 py-3 text-xs font-medium uppercase text-right ' + (isDarkMode ? 'text-gray-400' : 'text-gray-500')}>Acoes</th>
                </tr>
              </thead>
              <tbody className={'divide-y ' + (isDarkMode ? 'divide-white/10' : 'divide-gray-200')}>
                {employees.map((emp) => (
                  <tr key={emp.id} className={isDarkMode ? 'hover:bg-white/5' : 'hover:bg-gray-50'}>
                    ...
                    <td className="px-6 py-4"><span className={'px-3 py-1 rounded-full text-xs font-medium ' + getRoleColor(emp.role)}>{roleLabel(emp.role)}</span></td>
                    <td className="px-6 py-4"><button onClick={() => handleToggle(emp)} className={'px-3 py-1 rounded-full text-xs font-medium ' + (emp.isActive ? (isDarkMode ? 'bg-green-500/20 text-green-400' : 'bg-green-100 text-green-700') : (isDarkMode ? 'bg-red-500/20 text-red-400' : 'bg-red-100 text-red-700'))}>{emp.isActive ? 'Ativo' : 'Inativo'}</button></td>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className={'rounded-2xl p-6 w-full max-w-md border ' + (isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200')}>
              <h2 className={'text-xl font-bold mb-6 ' + (isDarkMode ? 'text-white' : 'text-gray-900')}>{editingEmployee ? 'Editar Funcionario' : 'Novo Funcionario'}</h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div><label className={'block text-sm font-medium mb-2 ' + (isDarkMode ? 'text-gray-300' : 'text-gray-700')}>Usuario de Login *</label><input type="text" value={formData.username} onChange={(e) => setFormData({ ...formData, username: e.target.value })} className={inputCls} placeholder="Ex: joao.silva" required disabled={!!editingEmployee} /></div>
                <div><label className={'block text-sm font-medium mb-2 ' + (isDarkMode ? 'text-gray-300' : 'text-gray-700')}>{editingEmployee ? 'Nova Senha (deixe vazio)' : 'Senha *'}</label><input type="password" value={formData.password} onChange={(e) => setFormData({ ...formData, password: e.target.value })} className={inputCls} placeholder="Minimo 6 caracteres" required={!editingEmployee} /></div>
                <div><label className={'block text-sm font-medium mb-2 ' + (isDarkMode ? 'text-gray-300' : 'text-gray-700')}>Nome Completo *</label><input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className={inputCls} placeholder="Ex: Joao da Silva" required /></div>
                <div><label className={'block text-sm font-medium mb-2 ' + (isDarkMode ? 'text-gray-300' : 'text-gray-700')}>Email *</label><input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className={inputCls} placeholder="joao@empresa.com" required /></div>
                <div><label className={'block text-sm font-medium mb-2 ' + (isDarkMode ? 'text-gray-300' : 'text-gray-700')}>Cargo</label><select value={formData.role} onChange={(e) => setFormData({ ...formData, role: e.target.value })} className={selectCls}><option value="seller">Vendedor</option><option value="manager">Gerente</option><option value="admin">Administrador</option><option value="viewer">Visualizador</option></select></div>
